...
class VerifyOTP extends React.Component {
  ...
  verifyNumber = () => {
    ...
    let urr = 'https://2factor.in/API/V1/5b1a9203-8f78-11ea-9fa5-0200cd936042/SMS/VERIFY/' + this.state.session_id + '/' + this.state.otp;
    fetch(urr)
      .then(response => response.json())
      .then((responseJson) => {
        if (responseJson.Status == 'Success') {
          console.log(responseJson)
          this.props.navigation.navigate('ResetPassword', { mobile_number: this.state.mobile_number });
        } else {
